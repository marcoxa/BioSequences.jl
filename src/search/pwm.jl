...
### ============================================
###
### This file defines two types of matrices: PFM and PWM.  PFM is a
### thin wrapper of Matrix and stores non-negative frequency values
### for each symbol and position. PWM is a position weighted matrix
### and stores score values like PFM.  PFMs are mutable like usual
### matrices but PWMs cache partial scores to each position and hence
### must be immutable so that updating operations won't cause
### inconsistency.  PFM is created from a raw frequency matrix or a
### set of sequences. PWM is created from a raw score matrix or a PFM.
###
### The motif search algorithm implemented here uses PWM. It tries to
### find the nearest position from which the total score exceeds a
### specified threshold. The algorithm stops searching immediately
### when it finds that the total score will not exceeds the threshold
### even if the maximum partial score is achieved from that
### position. This uses the partial score cache stored in a PWM
### object.
...
function Base.similar(bc::Broadcast.Broadcasted{PFMBroadcastStyle{S}},
                      elt::Type{T}) where {S, T}
...
Create a position weight matrix from a position frequency matrix
`pfm`.

The positive weight matrix will be
`log2.((pfm ./ sum(pfm, 1)) ./ prior)`.
"""
...
### TODO: Have one version of search_nuc that searches backwards as
### well as forwards?

function search_nuc(seq::BioSequence,
                    first::Int,
                    last::Int,
                    pwm::PWM{<: Union{DNA, RNA}, S},
                    threshold::S) where {S <: Real}
    check_pwm(seq, pwm)
...
function rsearch_nuc(seq::BioSequence,
                     first::Int,
                     last::Int,
                     pwm::PWM{<: Union{DNA, RNA}, S},
                     threshold::S) where {S <: Real}
...
function search_nuc2(seq::BioSequence,
                     first::Integer,
                     last::Integer,
                     pwm::PWM{<: Union{DNA, RNA}, S},
                     threshold::S) where {S <: Real}
    ...
    pwmlen = size(pwm, 2)
    interval′ = range(ifelse(rev, first - pwmlen + 1, first),
                      step = ifelse(rev, -1, 1),
...
function index_nuc(s::Union{DNA,RNA}, j::Integer)
    if !iscertain(s)
        throw(ArgumentError(string("index symbol must be A, C, G or ",
                                   typeof(s) == DNA ? "T" : "U")))
    end
...
function show_nuc_matrix(io::IO, m::Union{PFM{S}, PWM{S}}) where
    {S <: Union{DNA, RNA}}
    
    compact(x) = string(x ≥ 0 ?
        " " :
        "", sprint(show, x, context = :compact => true))
...
function PWMSearchQuery(sequences,
                        threshold::Real,
                        prior = fill(1 / 4, 4))
    pfm = PFM(sequences)
...
function Base.findnext(query::PWMSearchQuery, seq::BioSequence, start)
    if eltype(seq) == DNA || eltype(seq) == RNA
        return search_nuc(seq,
                          start,
                          lastindex(seq),
                          query.pwm,
                          convert(eltype(query.pwm), query.threshold))
    else
