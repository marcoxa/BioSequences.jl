...
Using these functions with these queries allows you to search a given
sequence for a sub-sequence, whilst allowing a specific number of
errors.

In other words they find a subsequence of the target sequence within a
specific [Levenshtein
distance](https://en.wikipedia.org/wiki/Levenshtein_distance) of the
query sequence.
...
Construct an [`ApproximateSearchQuery`](@ref) predicate for use with
Base find functions.
...
"""
function ApproximateSearchQuery(pat::S,
                                comparator::F = isequal) where
    {F <: Function, S <: BioSequence}
...
Return the range of the first occurrence of `pat` in `seq[start:stop]`
allowing up to `k` errors.
...
"""
function Base.findnext(query::ApproximateSearchQuery,
                       k::Integer,
                       seq::BioSequence,
                       start::Integer)
...
Return the range of the last occurrence of `query` in
`seq[stop:start]` allowing up to `k` errors.

Symbol comparison is done using the predicate supplied to the query.
By default, `ApproximateSearchQuery`'s predicate is `isequal`.
"""
function Base.findprev(query::ApproximateSearchQuery,
                       k::Integer,
                       seq::BioSequence,
                       start::Integer)
...
function Base.findfirst(query::ApproximateSearchQuery,
                        k::Integer,
                        seq::BioSequence)
    findnext(query, k, seq, firstindex(seq))
end


function Base.findlast(query::ApproximateSearchQuery,
                       k::Integer,
                       seq::BioSequence)
    findprev(query, k, seq, lastindex(seq))
end


function Base.occursin(query::ApproximateSearchQuery,
                       k::Integer,
                       seq::BioSequence)
    !isnothing(_approxsearch(query, k, seq, 1, lastindex(seq), true))
end


function _approxsearch(query::ApproximateSearchQuery,
                       k::Integer,
                       seq::BioSequence,
                       start::Integer,
                       stop::Integer,
...
### This returns the end index of a suffix sequence with up to `k`
### errors.  More formally, when `forward = true`, it returns the
### minimum `j ∈ start:stop` such that `min_{g ∈ 1:j} δ(pat, seq[g:j])
### ≤ k` where `δ(s, t)` is the edit distance between `s` and `t`
### sequences. See Myers' paper for details: Myers, Gene. "A fast
### bit-vector algorithm for approximate string matching based on
### dynamic programming." Journal of the ACM (JACM) 46.3 (1999):
### 395-415.  NOTE: `Pcom` corresponds to `Peq` in the paper.

function search_approx_suffix(Pcom::Vector{UInt64},
                              pat::BioSequence,
                              seq::BioSequence,
                              k::Integer,
                              start::Integer,
...
### Run dynamic programming to get the starting position of the
### alignment.

function alignback!(query::ApproximateSearchQuery{<: Function, <: BioSequence},
                    seq::BioSequence,
                    dist::Int,
                    start::Integer,
                    matchstop::Integer,
...
### ApproxSearchQuery.jl ends here.
