...
In-place copy content of `src` to `dst`, resizing `dst` to fit.  The
alphabets of `src` and `dst` must be compatible.
...
```
"""
function Base.copy!(dst::SeqOrView{A},
                    src::SeqOrView{A}) where {A <: Alphabet}
...
function _copyto!(dst::SeqOrView{A}, doff::Integer,
    ...
    ## This prevents a sequence from destructively overwriting its own
    ## data.
...
In-place copy content of sequence-like object `src` to `dst`, resizing
`dst` to fit.  The content of `src` must be able to be encoded to the
alphabet of `dst`.
...
function Base.copy!(dst::SeqOrView{<: Alphabet},
                    src::ASCIILike,
                    C::AsciiAlphabet)
...
### This is used to effectively scan an array of UInt8 for invalid
### bytes, when one is detected.

@noinline function throw_encode_error(A::Alphabet,
...
@inline function encode_chunk(A::Alphabet,
                              src::AbstractArray{UInt8},
                              soff::Integer,
                              N::Integer)
    chunk = zero(UInt64)
...
### Use this for AsiiAlphabet alphabets only, internal use only, no
### boundschecks.  This is preferential to `copyto!` if none of the
### sequence's original content needs to be kept, since this is
### faster.
...
function encode_chunks!(dst::SeqOrView{A},
    ...
    @inbounds for i in startindex:startindex+chunks-1
        dst.data[i] =
            encode_chunk(A(), src, soff, symbols_per_data_element(dst))
...
function Base.copyto!(dst::SeqOrView{<:Alphabet},
                      src::ASCIILike,
                      ::AsciiAlphabet)
    len = ncodeunits(src)
...
In-place encode `N` elements from `src` starting at `soff` to `dst`,
starting at `doff`.
The length of `dst` must be greater than or equal to `N + doff - 1`.
The first N elements of `dst` are overwritten,
the other elements are left untouched. The content of `src` must be
able to be encoded to the alphabet of `dst`.
...
function Base.copyto!(dst::SeqOrView{A},
                      doff::Integer,
                      src::SeqLike,
...
### For ASCII seq and src, convert to byte vector and dispatch using
### that.
...
function Base.copyto!(dst::SeqOrView{A},
                      ...
                      soff::Integer,
                      N::Integer,
                      C::AsciiAlphabet) where {A <: Alphabet}
    return Base.copyto!(dst, doff, codeunits(src), soff, N, C)
end


@noinline function throw_enc_indexerr(N::Integer,
                                      len::Integer,
                                      soff::Integer)
...
function Base.copyto!(dst::SeqOrView{A}, doff::Integer,
    ...
    length(src) < soff + len - 1 &&
        throw_enc_indexerr(len, length(src), soff)
...
function Base.copyto!(dst::SeqOrView{A}, doff::Integer,
                      src::AbstractVector{UInt8}, soff::Integer,
                      N::Integer,
    ...
    ## Fill in first chunk. Since it may be only partially filled from
    ## both ends, bit-tricks are harder and we do it the
    ## old-fashined way.  Maybe this can be optimized but eeehhh...
    ...
    @inbounds if !iszero(remaining)
        ...
        before =
            dst.data[index(bitind)] &
            (typemax(UInt) << (remaining * bits_per_symbol(A())))
