...
`Alphabet` is the most important type trait for `BioSequence`.

An `Alphabet` represents a set of biological symbols encoded by a
sequence, e.g. A, C, G and T for a DNA Alphabet that requires only 2
bits to represent each symbol.

# Extended help

*  Subtypes of Alphabet are singleton structs that may or may not be
   parameterized. 
*  Alphabets span over a *finite* set of biological symbols.
*  The alphabet controls the encoding from some internal "encoded data"
   to a BioSymbol of the alphabet's element type, as well as the
   decoding, the inverse process. 
*  An `Alphabet`'s `encode` method must not produce invalid data. 

Every subtype `A` of `Alphabet` must implement:
* `Base.eltype(::Type{A})::Type{S}` for some eltype `S`, which must be
a `BioSymbol`.
* `symbols(::A)::Tuple{Vararg{S}}`. This gives tuples of all symbols
  in the set of `A`.
* `encode(::A, ::S)::E` encodes a symbol to an internal data eltype
  `E`.
* `decode(::A, ::E)::S` decodes an internal data eltype `E`
   to a symbol `S`.
* Except for `eltype` which must follow Base conventions, all
  functions operating on `Alphabet` should operate on instances of the
  alphabet, not the type.

If you want interoperation with existing subtypes of `BioSequence`,
the encoded representation `E` must be of type `UInt`, and you must
also implement:
...
"""
The number of bits required to represent a packed symbol encoding in a
vector of bits.
"""
...
"""
Compute whether all bitpatterns represent valid symbols for an
alphabet.
""" 
...
function Base.showerror(io::IO, err::EncodeError{A}) where {A}
    print(io, "cannot encode ", err.val, " in ", A) # Maybe @error ?
end
...
"""
Alphabet of nucleic acids. Parameterized by the number of bits per
symbol, by default only `2` or `4`-bit variants exists.
"""
...
`RNAAlphabet` has a parameter `N` which is a number that determines
...
for A in (DNAAlphabet, RNAAlphabet)
	...
	## 2-bit encoding
        @inline function encode(::$(A){2}, nt::$(T))
            ...
            return convert(UInt,
                           @inbounds twobitnucs[reinterpret(UInt8, nt) + 0x01])
        end
...
for alph in (DNAAlphabet, RNAAlphabet)
    @eval function Base.promote_rule(::Type{A}, ::Type{B}) where
        {A <: $alph, B <: $alph}
...
Encode the ASCII character represented by `b` to the internal alphabet
encoding.  For example, the input byte `UInt8('C')` is encoded to
`0x01` and `0x02` for 2- and 4-bit DNA alphabets, reprectively.  This
method is only needed if the `Alphabet` is an `AsciiAlphabet`.
