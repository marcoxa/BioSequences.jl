...
Base.@propagate_inbounds function Base.getindex(x::BioSequence,
...
Base.@propagate_inbounds function Base.setindex!(x::BioSequence,
                                                 v,
                                                 i::Integer)
    @boundscheck checkbounds(x, i)
...
Base.@propagate_inbounds function
    ...
    @boundscheck if length(x) != length(locs)
        throw(DimensionMismatch("Attempt to assign $(length(x)) "
                                * "values to $(length(locs)) destinations"))
    end
