...
@inline function reversebits(x::T, ::BitsPerSymbol{2}) where
    T <: Base.BitUnsigned
...
@inline function reversebits(x::T, ::BitsPerSymbol{4}) where
    T <: Base.BitUnsigned
...
@inline function complement_bitpar(x::Unsigned, ::T) where
    {T <: NucleicAcidAlphabet{2}}
    return ~x
end
...
@inline function complement_bitpar(x::Unsigned, ::T) where
    ...
    return (((x & repeatpattern(typeof(x), 0x11)) << 3) |
            ((x & repeatpattern(typeof(x), 0x88)) >> 3) |
            ((x & repeatpattern(typeof(x), 0x22)) << 1) |
            ((x & repeatpattern(typeof(x), 0x44)) >> 1)
            )
end
...
@inline function mismatch_bitcount(a::UInt64, b::UInt64, ::T) where
    {T <: NucleicAcidAlphabet{4}}
...
@inline function match_bitcount(a::UInt64, b::UInt64, ::T) where
    {T <: NucleicAcidAlphabet{4}}
...
@inline function ambiguous_bitcount(x::UInt64, ::T) where
    {T <: NucleicAcidAlphabet{4}}
    return count_nonzero_nibbles(enumerate_nibbles(x) &
        0xEEEEEEEEEEEEEEEE)
end
...
@inline function gap_bitcount(x::UInt64, ::T) where
    {T <: NucleicAcidAlphabet{4}}
    return count_0000_nibbles(x)
end


@inline function gap_bitcount(a::UInt64, b::UInt64, ::T) where
    {T <: NucleicAcidAlphabet{4}}
...
@inline function certain_bitcount(x::UInt64, ::T) where
    {T <: NucleicAcidAlphabet{4}}
...
@inline function certain_bitcount(a::UInt64, b::UInt64, ::T) where
    {T <: NucleicAcidAlphabet{4}}
